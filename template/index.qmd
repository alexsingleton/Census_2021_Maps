...
Census_Metadata %>% 
  ...
  unique() %>%
  mutate(Download = cell_spec("Download", "html", link = paste0(Table_ID,".csv"))) %>%
...
my_map <- OA %>%
    ...
    tm_borders(col = "#D3D3D3", lwd = 0.5, group = NULL) +  # OA border colour and weight
    
    tm_layout(frame = FALSE, # No frame around the map
              attr.outside = TRUE, # Needed to move the credits and scale bar outside the map 
    ...
    tm_credits("Contains OS and National Statistics data © Crown copyright and database right 2022",position = c("right", "top")) # Add the required credits for the data
...
One of the nice features of tmap is that it is also incredibly easy to make a simple interactive map. 
